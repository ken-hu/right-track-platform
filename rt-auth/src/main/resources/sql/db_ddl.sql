...
COMMENT ON COLUMN public.tenant.id IS '唯一ID';
COMMENT ON COLUMN public.tenant.tenant_code IS '租户编码';
COMMENT ON COLUMN public.tenant.tenant_name IS '名称';
COMMENT ON COLUMN public.tenant.status IS '状态 enabled;disabled;';
COMMENT ON COLUMN public.tenant.owner_user_id IS '管理员id';
COMMENT ON COLUMN public.tenant.contact_email IS '管理员联系邮箱';
COMMENT ON COLUMN public.tenant.contact_phone IS '管理员联系电话';
COMMENT ON COLUMN public.tenant.ext IS '扩展字段，存储租户的自定义配置或额外信息';
